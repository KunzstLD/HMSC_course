setwd("P:/h572/hmsc_course/kunz_stefan/data")
# OR ON A MAC:
# setwd("/Volumes/group/h572/hmsc_course/kunz_stefan/data")
...
SXY <- read.csv(file.path(getwd(), "SXY.csv"),
...
if(is.TP) {
  ...
  Tr = TP[, grep("feed.*|ovip.*|resp.*|volt.*", names(TP))]
  P = NULL
  # Check that the data looks as it should!
  View(Tr)
  View(P)
...
Tax[Tax=="psychodidae"] = "Psychodidae"
Tax[which(Tax[,3]!=""&Tax[,4]!=""), 4] = names(Y)[which(Tax[,3]!=""&Tax[,4]!="")]
Tax[Tax[,3]=="",3] = paste(Tax[Tax[,3]=="",2], "dG", sep="_")
Tax[Tax[,4]=="",4] = paste(Tax[Tax[,4]=="",3], "dS", sep="_")
Tax = data.frame(Tax)
Tax$Yspecies<-names(Y)

library(ape)
P = as.phylo(~Order/Family/Genus/Species, data = Tax, collapse = FALSE)
P$edge.length = rep(1, length(P$edge))
plot(P, cex=0.5)
P$tip.label[match(Tax$Species, P$tip.label)]<-Tax$Yspecies
plot(P, cex=0.5)

Y = as.matrix(Y)
save(S,X,Y,Tr,P,Tax, file="allData.R")
